...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO borrow_log (book_id, borrower_name, borrower_email, borrower_gender, book_author, book_genre, book_status, book_isbn, borrow_date) VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW())");
        $stmt->bind_param("ssssssss", $bookId, $borrowerName, $borrowerEmail, $borrowerGender, $bookAuthor, $bookGenre, $bookStatus, $bookISBN);
        $stmt->execute();
